...
//*************************************************************************	
	...
	{
	 	if(player["state"]='ready') return true; // to do !
		else return false;
	};
	...
	if (!player["loading"]) {
		...
		//if(!player.delayload) player["load"](url); // external file function-- for minify
		//console.log(player.state+" loading here ");
	...
	Acts.prototype.Go_back = function ()
	{
		player["jump"](-1);
		return false;
	};
	Acts.prototype.Go_fwd = function ()
	{
		player["jump"](1);
		return false;
	};
  
	Acts.prototype.set_stereo = function (sp)
	{
		player["setseparation"](sp);
  ...
  Acts.prototype.Setvolume = function (svol)
	{
		player["volumeNode"]["gain"]["value"]=(svol);
	};
